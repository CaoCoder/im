    ...
    : QScrollArea {parent}
{
    ...
    QScrollBar:vertical {
        width: 2px; 
        background-color: rgb(43,43,43); /* ; /* 滚动条的轨道 */
    }
    ...
    : owner(owner)
{
    ...
    this->setStyleSheet("QWidget { background-color: rgb(24, 24, 24); }");
    ...
    nameLabel->setStyleSheet("QLabel { font-size: 12px; font-weight: 600; color: rgb(200 , 200 , 200); }");
    ...
    messageLabel->setStyleSheet("QLabel {  color: rgb(91, 91, 91); }");
...
void SessionFriendItem::enterEvent(QEnterEvent *event)
{
    ...
    // 设置一个更深的颜色
    this->setStyleSheet("QWidget { background-color: rgb(42, 42, 42);}");
}
...
void SessionFriendItem::select()
{
    ...
    for (QObject* child : children) {
        ...
        if (item->selected) {
            item->selected = false;
            item->setStyleSheet("QWidget { background-color: rgb(24, 24, 24); }");
        }
    }

    // 点击时, 修改背景色.
    // 此处不仅仅要设置当前 item 背景色, 也要还原其他元素的背景色.
    this->setStyleSheet("QWidget { background-color: rgb(37, 37, 37); }");
    this->selected = true;
